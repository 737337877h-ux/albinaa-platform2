...
## [1.2.1] - 2026-08-01 — صفحة الإشعارات الكاملة (Web Hotfix)

### الإضافات الجديدة
- **صفحة إشعارات كاملة** بدل الصفحة المؤقتة — قائمة الإشعارات مع: النوع (وعد مستحق/متأخر، تحصيل جديد، نقل عميل)، الوصف التفصيلي (العميل، المبلغ/العملة، التاريخ، السبب)، حالة القراءة، رابط الانتقال لبطاقة العميل
- **تحديد كمقروء** — زر لكل إشعار غير مقروء (PATCH `/notifications/:id/read`)
- **تحديد الكل كمقروء** — زر في رأس الصفحة (PATCH `/notifications/read-all`)
- **فلتر غير المقروءة فقط** مع عدّاد الإشعارات غير المقروءة
- **ترقيم صفحات** (25 لكل صفحة) عبر واجهة `/notifications` الموجودة

### الإصلاحات (Stabilization)
- **Bell Sync**: عدّاد الجرس في الشريط العلوي يتحدّث فورًا بعد قراءة أي إشعار (invalidate لاستعلامي `notifications` و`notifications-preview`)

### الموثوقية
- بدون تغييرات في الـSchema أو الـMigration أو الاعتماديات
- backend/frontend typecheck + lint نظيفان · Web Dashboard: **1.2.1** · Backend: **1.2.1**
