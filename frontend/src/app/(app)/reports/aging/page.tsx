...
export default function AgingReportPage() {
  ...
  const currencies = Object.keys(report.data?.totals ?? {});
  const rows = useMemo(() => (report.data?.customers ?? []).filter((row) =>
    (currency === 'ALL' || row.currency === currency) && (!activeBucket || row.buckets[activeBucket] > 0),
  ), [report.data, currency, activeBucket]);
  ...
  return <div className="space-y-5">
    ...
    <DataState isLoading={report.isLoading} isError={report.isError} error={report.error} onRetry={() => report.refetch()} isFetching={report.isFetching} isEmpty={!report.data?.customers.length} emptyTitle="لا توجد مديونية موجبة" skeletonClassName="h-64">
      <div className="space-y-5">
        {currencies.filter((code) => currency === 'ALL' || code === currency).map((code) => {
          ...
          return <Card key={code} className="p-4">
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
              {BUCKETS.map(([bucket, label]) => <button key={bucket} type="button" onClick={() => setActiveBucket(activeBucket === bucket ? null : bucket)} className={`rounded-lg border-r-4 p-3 text-right transition hover:-translate-y-0.5 ${heat(bucket)} ${activeBucket === bucket ? 'ring-2 ring-pine-500' : ''}`}>
