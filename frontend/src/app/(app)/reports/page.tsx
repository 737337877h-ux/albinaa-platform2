...
export default function ReportsPage() {
  ...
  const report = useQuery({ queryKey: ['reports-summary'], queryFn: () => api<ReportSummary>('/reports/summary'), enabled: allowed && hasToken() });
  ...
  async function exportExcel() {
    setDownloading(true);
    try { await downloadApiFile('/reports/summary.xlsx', `albinaa-report-${new Date().toISOString().slice(0, 10)}.xlsx`); toast('تم تنزيل تقرير Excel المنسق', 'ok'); }
  ...
  return <div className="report-print space-y-5">
    ...
    <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-concrete-500">ملخص المديونية والتحصيل • كل عملة مستقلة • {report.data ? new Date(report.data.generatedAt).toLocaleString('ar-YE') : '—'}</p><Select aria-label="العملة" className="w-44" value={selected} onChange={(event) => setCurrency(event.target.value)}>{currencies.map((code) => <option key={code}>{code}</option>)}</Select></div>
