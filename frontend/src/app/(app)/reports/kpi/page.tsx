...
interface TrendRow { month: string; currency: string; opening: number; sales: number; closing: number; dso: number | null; cei: number | null; averageDebtAge: number | null; undatedDebt: number; closedSnapshot: boolean }
...
export default function KpiPage() {
  ...
  const report = useQuery({ queryKey: ['reports-kpi'], queryFn: () => api<KpiResponse>('/reports/kpi'), enabled: allowed && hasToken() });
  ...
  return <div className="space-y-5">
    ...
    <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-concrete-500">اتجاه 12 شهرًا • العملات منفصلة • آخر تحديث {report.data ? new Date(report.data.generatedAt).toLocaleString('ar') : '—'}</p><Select aria-label="العملة" className="w-40" value={selected} onChange={(event) => setCurrency(event.target.value)}>{currencies.map((code) => <option key={code}>{code}</option>)}</Select></div>
    <DataState isLoading={report.isLoading} isError={report.isError} error={report.error} onRetry={() => report.refetch()} isFetching={report.isFetching} isEmpty={!currencies.length} emptyTitle="لا توجد بيانات مؤشرات بعد" skeletonClassName="h-64">
      {latest && <div className="space-y-5">
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <Metric title="CEI — كفاءة التحصيل" value={latest.cei === null ? 'بانتظار الإقفال' : `${fmtMoney(latest.cei)}%`} hint={latest.closedSnapshot ? 'مبني على لقطة شهرية' : 'لا توجد لقطة شهرية'} icon={<Target className="h-4 w-4" />} />
        ...
        {report.data!.dataQuality.monthsWithoutAgingSnapshot.length > 0 && <Card className="border-r-4 border-r-hazard-500 p-4 text-sm"><p className="font-semibold">جودة المؤشرات</p><p className="mt-1 text-concrete-500">CEI ومتوسط العمر يظهران «بانتظار الإقفال» للأشهر بلا لقطة Aging؛ لم تُستخدم أرقام تقديرية.</p></Card>}
      </div>}
    </DataState>
